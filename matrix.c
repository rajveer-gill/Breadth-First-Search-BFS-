...
void matrix_transpose(int** dst, int** src, int rows, int cols)
{
	...
	for(int i = 0; i < rows; i++){
		for(int j = 0; j < cols; j++){
			hold = src[i][j];//get value
			dst[j][i] = hold;//store value in matrix
		}
	}
}
              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        ...
        // HINT: spmv 
	int nf = 0;//tracks neighbors found
        ...
        // the source vertex)
	reset_vector(dst, cols);//make vector all 0s
	int n;//tracks neighbor for source vertex
	for(int i = 0; i < cols; i++){
		if(src[i] == 1){//checks if source vector exists
			for(int x = 0; x < rows; x++){
				n = mat_trans[x][i] * src[i];
				if(n){//checks if neighbor exists
					nf = 1;//set neighbors found to true
					dst[x] = 1;//update dst
				}
			}
		}
	...
	int hold;
	for(int i = 0; i < rows; i++){
		if(dst[i] == 1){//checks if source vertices are true
			color[i] = 2;//set color to visited
			distance[i] = iter;//update distance
		}
		hold = dst[i];//hold source
		src[i] = hold;//update source
		if(color[i] == 2){//check if vertices are visited
			for(int x = 0; x < cols; x++){
				mat_trans[i][x] = 0;//eliminate vertices visited
			}
		}
	}
	if(!nf){//checks if neighbors not found
		done = 1;
	}
	iter++;//update distance from source
    }
