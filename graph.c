...
/* This function constructs an adjacency list for a graph from an adjacency
   ...
   input parameters: 
       ...
       The following are 'produced' by this function
       a_node_t*** list	a linked list of adjacencies
...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
	...
	for(int i = 0; i < rows; i++){//go through rows
		for(int j = 0; j < cols; j++){//go through cols
			if(adj_mat[i][j] == 1){//check for edge
				add_node(myList, i, create_node(j));//create node and add it to adj list
			}
		}
	}
	*list = myList;//copy list over to inputted list
         ...
         int* color, int* distance, int* parent)
{
	...
	adj_node_t** q = NULL;//make queue
	init_adj_list(&q, rows);//initialize queue
	assert(q);
	int qCount = 0;//make counter for items in queue
	add_node(q, 0, create_node(source));//add source node to queue
	qCount++;
    // bfs iteration
    // HINT: use remove_node (to fetch & dequeu the vertex)
    // HINT: you will also need create_node an add_node here
	while(qCount > 0){//check if queue is empty
		int removedNode = remove_node(q);//remove next node in queue
		qCount--;
		adj_node_t* curNode = list[removedNode];
		while(curNode != NULL){//check if node exists
			int vidHold = curNode -> vid;
			if(color[vidHold] == 0){//check if node is visited
				color[vidHold] = 1;//show node is in queue
				distance[vidHold] = distance[removedNode] + 1;//update distance
				parent[vidHold] = removedNode;//update parent
				add_node(q, 0, create_node(vidHold));//add new node to queue
				qCount++;
			}
			curNode = curNode -> next;//update curNode to the next node it is pointing to
		}
		color[removedNode] = 2;//show node has been visited
	}
	free_adj_list(q, rows);//free queue
   fprintf(stdout, "done\n");
